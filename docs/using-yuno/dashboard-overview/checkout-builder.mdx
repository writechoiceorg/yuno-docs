...
### Billing / Shipping Address — Required Fields
...
2. In **Payment method settings**, click the **three dots (⋯)** on the payment method you want to configure.
3. Open the **Required Fields** drawer.
4. Find the **Address configuration** section.
...
4. Return to the Checkout Builder and click **Publish** for the changes to take effect in the live checkout.
...
- ⚠️ **Configure during low-traffic periods:** Publishing changes to required fields affects all subsequent checkout sessions immediately. Customers in the middle of an in-flight session may see inconsistent forms during the propagation window. We recommend configuring and publishing this setting during low-traffic periods to minimize impact.

In the Required fields, you can also configure the behavior for enrolled cards, such as requesting the CVV for every transaction or only during the first payment.
