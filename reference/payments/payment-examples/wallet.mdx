...
Example of a request for creating a payment using DolarApp enrollment. Below are examples of a request and the received response for successful payment creation. The request is presented using the cURL format, and the response is a JSON object.
...
```json Response (JSON)
{
    ...
    "transactions": {
        ...
        "created_at": "2026-06-03T15:46:05.167780Z",
...
<Note>
The payment is created with `status: PENDING` / `sub_status: PENDING_PROVIDER_CONFIRMATION`. The customer approves the payment in the DolarApp (ARQ) application; Yuno then delivers the final status via webhook. There is no redirect step.
</Note>
